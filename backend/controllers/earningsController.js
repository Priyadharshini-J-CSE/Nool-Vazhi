...
const getDriverEarnings = async (req, res) => {
  ...
  try {
    ...
    const totalTripsToday = todayBookings.length;

    const pendingPayments = allBookings
      .filter(b => !b.remainingPaid)
      .reduce((s, b) => {
        const remaining = b.totalPrice - (b.advancePaid ? b.advanceAmount : 0);
        return s + remaining;
      }, 0);
    ...
    res.json({
      summary: { todayEarnings, totalTripsToday, pendingPayments },
      rows,
    });
  } catch (err) {
