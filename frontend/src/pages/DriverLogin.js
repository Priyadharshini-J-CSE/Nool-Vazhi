...
export default function DriverLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await authAPI.loginDriver(form);
      login(data);
      navigate('/dashboard');
    } catch (err) {
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.card} className="fade-in">
        ...
        <p style={styles.sub}>Sign in to your driver account</p>
        ...
        <p style={styles.footer}>
          Not a driver? <Link to="/login/organization" style={styles.link}>Org Login</Link>
          {' · '}
          <Link to="/register/driver" style={styles.link}>Create Account</Link>
        </p>
      </div>
    </div>
  );
